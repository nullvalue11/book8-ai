#!/usr/bin/env python3
"""
Extended Business Registration Test - Test business confirm execution
"""
...
def test_business_confirm_execution():
    print("🔍 Testing Business Confirm Execution")
    print("=" * 50)
    
    # 1. Register user
    unique_id = str(uuid.uuid4())[:8]
    user_email = f"confirm-test-{unique_id}@example.com"
    
    register_data = {
        "email": user_email,
        "password": "TestPassword123!",
        "name": f"Confirm Test User {unique_id}"
    }
    ...
    # 3. Confirm business (execute provisioning)
    print(f"\n3. Confirming business {business_id}...")
    confirm_data = {"businessId": business_id}
    result = make_request('POST', '/api/business/confirm', confirm_data, headers)
    
    # 4. Check final business status
    print(f"\n4. Final business status check...")
    result = make_request('GET', f'/api/business/{business_id}', headers=headers)
    
    print(f"\n✅ Test complete for user: {user_email}")

if __name__ == "__main__":
    test_business_confirm_execution()