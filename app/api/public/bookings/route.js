...
export async function POST(req) {
  try {
    ...
    try {
      ...
      const overlap = busy.some(b => new Date(b.start) < new Date(slotEnd) && new Date(slotStart) < new Date(b.end))
      if (overlap) return NextResponse.json({ ok: false, error: 'Slot no longer available' }, { status: 409 })
    ...
    try {
      const evt = {
        ...
        attendees: [{ email: user.email }, { email: guest.email }],
      }
    ...
    try {
      ...
      await resend.emails.send({
        ...
        subject: 'Your Book8 meeting is confirmed',
        html: `<p>Hi ${guest.name || ''},</p><p>Your meeting is confirmed.</p><p>${new Date(booking.startTime).toLocaleString()} – ${new Date(booking.endTime).toLocaleString()} (${booking.timeZone})</p>`,
