...
async function handleSubscriptionEvent(event, stripe, database) {
  ...
  try {
    ...
    if (type === 'customer.subscription.deleted') {
      ...
      // Use canceledAt as the idempotency guard — if it's already set, this is a replayed
      // webhook and we MUST NOT re-send the end-of-access email or duplicate state writes.
      ...
      for (const biz of businessesForDelete) {
        ...
        if (!wasAlreadyCanceled) {
          ...
          // End-of-access email — only sent on the first deletion event.
          try {
          ...
          console.log(
            `[webhooks/stripe] ${type}: business ${bid} already canceled — skipping email + audit (idempotent replay)`
          )
        }
      }
