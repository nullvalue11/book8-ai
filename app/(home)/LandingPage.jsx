...
/** Official WhatsApp mark — keep #25D366 in light and dark themes (BOO-HOMEPAGE-V2-1B). */
...
function HeroChannelCallout({ h }) {
  const items = [
    { Icon: Phone, label: h.heroChannelPhone },
    { Icon: MessageSquare, label: h.heroChannelSms },
    { Icon: Link2, label: h.heroChannelLink },
    { whatsapp: true, label: h.heroChannelWhatsapp }
  ]
  return (
    <div className="mx-auto mb-2 mt-6 max-w-lg min-[480px]:max-w-none">
      <div className="grid grid-cols-2 justify-items-center gap-6 min-[480px]:grid-cols-4 min-[480px]:gap-4">
        {items.map((item) => {
          if (item.whatsapp) {
            return (
              <div key="whatsapp" className="flex flex-col items-center gap-2 text-center">
                <WhatsAppGlyph className="h-8 w-8 shrink-0" />
                <span className="text-xs text-slate-500 dark:text-[#9593A8]">{item.label}</span>
              </div>
            )
          }
          ...
          return (
            <div key={item.label} className="flex flex-col items-center gap-2 text-center">
              <Icon
                className="h-8 w-8 shrink-0 text-slate-700 dark:text-[#E8E6F2]"
                strokeWidth={1.75}
                aria-hidden
              />
              <span className="text-xs text-slate-500 dark:text-[#9593A8]">{item.label}</span>
